...
exports.edit = (request, response) ->
	...
	renderViewForEditShoes(shoesId, response)

exports.update = (request, response) ->
	shoesId = request.params.id
	
	if not request.form.isValid
		return renderViewForEditShoes(shoesId, response, request.form.getErrors())
	...
	step(
		...
		updateShoes = (error, pairOfShoes) ->
			if error
				throw new errors.DataError('An error occured while loading the data for updating a pair of shoes.', error)
		...
		redirectToIndex = (error) ->
			if error
				throw new errors.PersistenceError('An error occured while saving a pair of shoes in the data store.', error)

			response.redirect('/shoes')	
	)

renderViewForEditShoes = (shoesId, response, validationErrors) ->
	step(
		loadData = ->
		...
		renderView = (error, pairOfShoes) ->
			if error
				throw new errors.DataError('An error occured while loading data for the edit pair of shoes page.', error)
